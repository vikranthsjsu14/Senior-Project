    ...
    <div style={styles.page}>
      ...
      {logs.length > 0 && (
        <div style={styles.totals}>
          ...
          <Stat label="Protein" value={Math.round(totalProtein)} unit="g" color="var(--accent)" />
...
function Stat({ label, value, unit, color }: { label: string; value: number; unit: string; color: string }) {
  return (
    <div style={{ backgroundColor: 'var(--bg-card)', borderRadius: '10px', padding: '14px 20px', textAlign: 'center', flex: 1 }}>
      <div style={{ color: 'var(--text-muted)', fontSize: '12px', marginBottom: '4px' }}>{label}</div>
      <div style={{ color, fontSize: '22px', fontWeight: 700 }}>{value}<span style={{ fontSize: '13px', color: 'var(--text-muted)' }}> {unit}</span></div>
    </div>
  );
}
...
const styles: Record<string, React.CSSProperties> = {
  ...
  dateInput: { padding: '8px 12px', backgroundColor: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '8px', color: 'var(--text)', fontSize: '14px' },
  ...
  mealGroup: { backgroundColor: 'var(--bg-card)', borderRadius: '12px', padding: '16px 20px' },
  mealTitle: { color: 'var(--text)', margin: '0 0 12px', fontSize: '15px' },
  logItem: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid var(--bg-input)' },
  logBody: { flex: 1 },
  foodName: { color: 'var(--text)', fontSize: '14px' },
  macros: { display: 'flex', gap: '12px', flexWrap: 'wrap', color: 'var(--text-muted)', fontSize: '12px', marginTop: '2px' },
  deleteBtn: { backgroundColor: 'transparent', border: 'none', color: 'var(--text-dim)', cursor: 'pointer', fontSize: '14px', padding: '4px 8px' },
